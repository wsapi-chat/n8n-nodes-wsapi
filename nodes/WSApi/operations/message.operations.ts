...
export async function executeMessageOperation(
  ...
  // Operations that use 'to' field (send operations and editMessage)
  const operationsWithTo = [
    "sendText",
    "sendLink",
    "sendImage",
    "sendVideo",
  ...
  const to = operationsWithTo.includes(operation)
  ...
  switch (operation) {
    ...
    case "sendLink":
      ...
      const linkDescription = this.getNodeParameter(
        "linkDescription",
        i,
        "",
      ) as string;
      const jpegThumbnail = this.getNodeParameter(
        "jpegThumbnail",
        i,
        "",
      ) as string;
    ...
    case "sendImage":
      ...
      if (imageInputType === "base64") {
        ...
        imageBody.url = this.getNodeParameter("mediaUrl", i) as string;
      }
    ...
    case "sendVideo":
      ...
      if (videoInputType === "base64") {
        ...
        videoBody.url = this.getNodeParameter("mediaUrl", i) as string;
      }
    ...
    case "sendAudio":
      ...
      if (audioInputType === "base64") {
        ...
        audioBody.url = this.getNodeParameter("mediaUrl", i) as string;
      }
    ...
    case "sendVoice":
      ...
      if (voiceInputType === "base64") {
        ...
        voiceBody.url = this.getNodeParameter("mediaUrl", i) as string;
      }
    ...
    case "sendDocument":
      ...
      if (documentInputType === "base64") {
        ...
        documentBody.url = this.getNodeParameter("mediaUrl", i) as string;
      }
    ...
    case "sendSticker":
      ...
      const stickerMimeType =
        (this.getNodeParameter("mimeType", i, "") as string) || "image/webp";
      const stickerIsAnimated = this.getNodeParameter(
        "isAnimated",
        i,
        false,
      ) as boolean;
      ...
      if (stickerInputType === "base64") {
        stickerBody.data = this.getNodeParameter("mediaBase64", i) as string;
      } else {
        stickerBody.url = this.getNodeParameter("mediaUrl", i) as string;
      }
    ...
    case "deleteForMe":
      ...
      const deleteForMeSenderId = this.getNodeParameter(
        "senderId",
        i,
      ) as string;
      const deleteForMeChatId = this.getNodeParameter("chatId", i) as string;
      const isFromMe = this.getNodeParameter("isFromMe", i) as boolean;
      const deleteTimestamp = this.getNodeParameter(
        "deleteTimestamp",
        i,
      ) as string;
      if (!deleteTimestamp) {
        throw new Error(
          "Deletion timestamp is required to delete message for me.",
        );
      }
      ...
      await this.helpers.requestWithAuthentication.call(this, "WSApiApi", {
        ...
        body: {
          chatId: deleteForMeChatId,
          senderId: deleteForMeSenderId,
          isFromMe,
