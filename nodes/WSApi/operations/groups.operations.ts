...
export async function executeGroupsOperation(
  ...
  switch (operation) {
    ...
    case "updateParticipants":
      ...
      const updateParticipantsData = this.getNodeParameter(
        "participants",
        i,
      ) as IDataObject;
      const updatePhoneNumbers = (
        (updateParticipantsData.phoneNumbers as string) || ""
      )
        .split(",")
        .map((p) => p.trim())
        .filter((p) => p);
    ...
    case "handleJoinRequests":
      ...
      const requestParticipantsData = this.getNodeParameter(
        "participants",
        i,
      ) as IDataObject;
      const requestParticipants = (
        (requestParticipantsData.phoneNumbers as string) || ""
      )
        .split(",")
        .map((p) => p.trim())
        .filter((p) => p);
